...
int main(int argc, char* argv[]) {
    ...
    /* 3. Creo menú principal */
    /* 3.1. Creo barra de menú */
    ...
    /* 3.4. Creo subitems y seteo sus callbacks */
    menu_item = gtk_menu_item_new_with_label("Open");
    gtk_menu_shell_append(GTK_MENU_SHELL(file_menu), menu_item);
    g_signal_connect(menu_item, "activate", G_CALLBACK(on_menu_file_open_click), window);

    menu_item = gtk_menu_item_new_with_label("Exit");
    gtk_menu_shell_append(GTK_MENU_SHELL(file_menu), menu_item);
    g_signal_connect(menu_item, "activate", G_CALLBACK(on_menu_file_exit_click), NULL);

    menu_item = gtk_menu_item_new_with_label("About");
    gtk_menu_shell_append(GTK_MENU_SHELL(help_menu), menu_item);
    g_signal_connect(menu_item, "activate", G_CALLBACK(on_menu_help_about_click), window);

    /* 3.5. Agrego la barra de menú al container */
    gtk_box_pack_start(GTK_BOX(box), menu_bar, FALSE, FALSE, 0);

    /* 4. Creo drawing area para el reproductor */
    player_window = gtk_drawing_area_new();
    gtk_box_pack_start(GTK_BOX(box), player_window, TRUE, TRUE, 0);
    g_signal_connect(G_OBJECT(player_window), "realize", G_CALLBACK(on_player_window_realize), NULL);
    g_signal_connect(G_OBJECT(player_window), "draw", G_CALLBACK(on_player_window_draw), NULL);

    /* 5. Creo los botones de control de reproducción */
    /* 5.1. Creo container para los botones */
    ...
    g_signal_connect(playpause_button, "clicked", G_CALLBACK(on_playpause_click), NULL);
    g_signal_connect(stop_button, "clicked", G_CALLBACK(on_stop_click), NULL);

    /* 5.3. Agrego los botones al container */
...
/* Esta funcion es llamada cada vez que la ventana de video necesita ser redibujada (debido a daño, exposición,
 * cambio de tamaño, etc). Simplmente dibujamos un rectángulo negro para evitar que aparezca basura. */
gboolean on_player_window_draw(GtkWidget* widget, cairo_t* cr) {
    GtkAllocation allocation;

    /* Cairo es una biblioteca gráfica 2D que usamos para limpiar la ventana de video. */
...
void media_play(void) {
    GtkWidget* button_icon = gtk_image_new_from_icon_name("media-playback-pause", GTK_ICON_SIZE_BUTTON);
    gtk_button_set_image(GTK_BUTTON(playpause_button), button_icon);
    libvlc_media_player_play(media_player);
}

void media_pause(void) {
    GtkWidget* button_icon = gtk_image_new_from_icon_name("media-playback-start", GTK_ICON_SIZE_BUTTON);
    gtk_button_set_image(GTK_BUTTON(playpause_button), button_icon);
...
void on_menu_help_about_click(GtkWidget* widget, gpointer data) {
    ...
    gtk_show_about_dialog (GTK_WINDOW(data),
                            "program-name", "GTK and libVLC usage example",
                            "version", "1.0.0",
                            "copyright", "\xc2\xa9 2019 Javier F. Balloffet",
                            "license-type", GTK_LICENSE_MIT_X11,
                            "website", "https://github.com/jballoffet/programando-c-en-linux",
                            "website-label", "Git Repository",
                            "comments", "Program to demonstrate GTK and libVLC functions.",
                            "authors", authors,
                            "documenters", documenters,
                            "logo-icon-name", "media-playback-start",
                            NULL);
}
