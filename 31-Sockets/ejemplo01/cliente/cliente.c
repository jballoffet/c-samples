...
int main(int argc, char* argv[])
{
    ...
    // Declaro un puntero a una estructura donde gethostbyname() me retornará la
    // IP.
    ...
    memcpy(&(server_data.sin_addr), host_data->h_addr_list[0],
           host_data->h_length);
    server_data.sin_port = htons(server_port);
    memset(&(server_data.sin_zero), 0, 8);

    // Conecto el socket al servidor.
    status = connect(client_socket_fd, (struct sockaddr*)&server_data,
                     sizeof(server_data));
    if (status == -1)
    {
    ...
    // Recibo el mensaje de bienvenida y coloco el último caracter recibido como
    // NULL, por si acaso.
